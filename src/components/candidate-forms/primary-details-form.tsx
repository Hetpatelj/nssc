...
export function PrimaryDetailsForm({ userData, onNext }: PrimaryDetailsFormProps) {
    const { user } = useUser();
    ...
    const [photoFile, setPhotoFile] = useState<FileInfo>(null);
    const [signFile, setSignFile] = useState<FileInfo>(null);
    ...
    useEffect(() => {
        if (userData.photoUrl) {
            setPhotoFile({ name: 'photo.jpg', url: userData.photoUrl });
            setValue('photoUrl', userData.photoUrl);
        }
        if (userData.signUrl) {
            setSignFile({ name: 'signature.jpg', url: userData.signUrl });
             setValue('signUrl', userData.signUrl);
        }
    }, [userData, setValue]);

    const saveFileUrlToFirestore = async (fileUrl: string, fieldName: 'photoUrl' | 'signUrl') => {
        if (user && firestore) {
