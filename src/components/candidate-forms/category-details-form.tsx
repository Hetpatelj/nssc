...
export function CategoryDetailsForm({
  ...
  useEffect(() => {
    if (userData.domicileFileUrl) {
      setDomicileFile({ name: 'domicile.jpg', url: userData.domicileFileUrl });
      setValue('domicileFileUrl', userData.domicileFileUrl);
    }
    if (userData.disabilityCertFileUrl) {
      setDisabilityCertFile({ name: 'disability.jpg', url: userData.disabilityCertFileUrl });
      setValue('disabilityCertFileUrl', userData.disabilityCertFileUrl);
    }
  }, [userData, setValue]);

  const saveFileUrlToFirestore = async (fileUrl: string, fieldName: 'domicileFileUrl' | 'disabilityCertFileUrl') => {
      if (user && firestore) {
